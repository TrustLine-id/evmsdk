...
- ✅ **Upgradeable Support** - Dedicated base contract for proxy-based deployments (see [Integration constraints](#integration-constraints))
...
- On upgradeable contracts, inherit `TrustlinedUpgradeable` and call **`__Trustlined_init`** from your `initializer` (the `Trustlined` constructor does not run on the proxy).
...
contract MyDapp is Initializable, TrustlinedUpgradeable {

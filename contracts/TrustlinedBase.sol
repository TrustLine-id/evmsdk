...
/// @title Trustline's Shared Base Contract
/// @author Trustline
/// @notice Shared storage, events, and validation logic for Trustlined integrations
/// @dev Not intended for direct inheritance — use `Trustlined` or `TrustlinedUpgradeable`.
...
abstract contract TrustlinedBase {
    ...
    /// @dev Deploys or adopts a Validation Engine proxy. Called from `Trustlined`'s constructor or `TrustlinedUpgradeable`'s initializer.
    /// @param logic The Validation Engine logic contract address (used only if `proxy` is zero)
    /// @param proxy Optional Validation Engine proxy address. If non-zero, it must have been deployed atomically by `Trustlined` (not manually).
    function _initializeValidationEngine(address logic, address proxy) internal {

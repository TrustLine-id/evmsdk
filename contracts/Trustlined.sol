...
abstract contract Trustlined {
    ...
    constructor(address trustlineValidationEngineLogic, address trustlineValidationEngineProxy) {
        if (trustlineValidationEngineProxy != address(0)) {
            ...
            _assertContractAccount(trustlineValidationEngineProxy);
            _assertValidationEngine(trustlineValidationEngineProxy);
            require(
                IAccessControlDefaultAdminRules(trustlineValidationEngineProxy).defaultAdmin() == msg.sender,
                "Invalid validation engine admin"
            );
            validationEngine = IValidationEngine(trustlineValidationEngineProxy);

            emit ValidationEngineAdopted(address(this), trustlineValidationEngineProxy);
        } else {
            ...
            _assertContractAccount(trustlineValidationEngineLogic);
